    ...
    "location": {
        ...
        "items": [
            ...
            },
            {
                "id": "arrived_home",
                "name": "Я дома",
                "desc": "Нажать кнопку — отправить сообщение 'Я дома'",
                "actions": [
                    {"WFWorkflowActionIdentifier": "is.workflow.actions.getlocation", "WFWorkflowActionParameters": {}},
        ...
        "items": [
            {
                ...
                "desc": "Скопировал текст → нажал → получил перевод",
                "actions": [
        ...
        "items": [
            {
                "id": "car_bluetooth",
                "name": "Сел в машину",
                "desc": "Подключился Bluetooth → открыть навигатор",
                "actions": [
...
SYSTEM_PROMPT = """Ты эксперт по iOS Shortcuts. Твоя задача — генерировать валидные JSON структуры для файлов .shortcut.
...
Формат ответа — ТОЛЬКО валидный JSON:
{
  ...
  "actions": [
    {"WFWorkflowActionIdentifier": "is.workflow.actions.xxx", "WFWorkflowActionParameters": {}}
  ]
}

Используй реальные идентификаторы: getlocation, sendmessage, notification, setbrightness, openapp, getclipboard, text."""
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        if len(parts) >= 3:
            cat_id, item_id = parts[1], parts[2]
            if cat_id in TEMPLATES:
        ...
        if len(parts) >= 3:
            ...
            if cat_id in TEMPLATES:
                item = next((i for i in TEMPLATES[cat_id]["items"] if i["id"] == item_id), None)
                if item:
                    shortcut_data = {"name": item["name"], "description": item["desc"], "actions": item["actions"]}
                    ...
                    await query.message.reply_document(
                        ...
                        caption=f"✅ *{item['name']}*\n\n{item['desc']}\n\n📱 Нажми на файл → Добавить команду",
                        parse_mode="Markdown",
        ...
        await query.edit_message_text(
            "✨ *Своя команда*\n\nОпиши что должна делать команда.\n\n"
            "Примеры:\n• Каждое утро отправляй геолокацию\n"
            "• Когда подключаюсь к WiFi — включи фокус\n\n"
            "💎 Стоимость: *20 ⭐*\n\nНапиши задачу:",
            parse_mode="Markdown",
        ...
        if not history:
            await query.edit_message_text("📦 *Мои команды*\n\nПока пусто.", parse_mode="Markdown", reply_markup=back_to_main())
        else:
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 👇 ГЛАВНОЕ ИСПРАВЛЕНИЕ — добавил provider_token и start_parameter
    await update.message.reply_invoice(
        ...
        start_parameter="generate_shortcut",  # 👈 ОБЯЗАТЕЛЬНО!
    )
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_request:
        await update.message.reply_text("❌ Ошибка. Попробуй еще раз.", reply_markup=back_to_main())
        return

    msg = await update.message.reply_text("⚙️ Генерирую команду...")
    ...
    try:
        ...
        await update.message.reply_document(
            document=file_bytes,
            filename=filename,
            caption=f"✅ *{shortcut_data.get('name', 'Команда готова')}*\n\n{shortcut_data.get('description', '')}\n\n📱 Нажми на файл → Добавить команду",
            parse_mode="Markdown",
            reply_markup=back_to_main()
        )
        ...
        logger.error(f"Generation error: {e}")
        await msg.edit_text("❌ Ошибка генерации. Попробуй еще раз.", reply_markup=back_to_main())
